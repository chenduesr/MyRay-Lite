...
- 左侧主导航图标和文字整体往左收，修复上一版居中过头的问题。
- 去掉按钮、左侧导航、节点列表行和日志列表行的鼠标悬停变色。
- 保留当前页面选中状态和按钮按下反馈，让界面更稳定不跳色。

## v1.8.3

### UI 修复

- 关闭页面和设置分区整体淡入，修复点击导航或分区时页面闪一下的问题。
- 侧边栏导航按钮内容改为居中显示，并去掉按钮点击后的虚线焦点框。
- 节点筛选按钮去掉点击焦点残留，减少鼠标移入移出时的视觉迟滞感。
- 日志页工具按钮改为紧凑排列，避免按钮被挤出或看不全。
- 日志页整体宽度、间距和日志区域高度重新收拢，排版更稳定。
- 设置页左侧二级导航列加宽，并取消按钮最小宽度导致的右侧裁切。

## v1.8.2

### 界面精修

- 节点列表新增当前节点标记、状态徽章、延迟胶囊和 hover 高亮，批量测速时状态反馈更清楚。
- 节点详情抽屉加宽并保留快捷操作，信息面板排版更舒展。
- 日志页新增日志行数、ERROR/WARN/XRAY 徽章和“复制错误”快捷操作。
- 日志空状态改为更友好的提示，并统一日志行 hover 反馈。
- 增加页面切换、设置分区、详情抽屉和 Toast 的轻微动效。
- 抽出统一 Badge、列表行和日志行样式，让卡片、徽章、间距更一致。

## v1.8.1

### 界面精修

- 首页重排为仪表盘样式，突出连接状态、主操作按钮和关键运行指标。
- 节点页新增筛选标签：全部、可用、低延迟、不可用、当前。
- 节点列表新增协议和传输方式小徽章，信息层级更清楚。
- 设置页改为左侧二级导航，基础、代理、路由、DNS、高级、更新分区更稳定。
- 日志页新增全部日志、应用日志、Xray 日志来源切换。
